...
func (s *ThumbnailService) GenerateThumbnail(asset *common_models.PHAsset, content []byte) ([]byte, error) {
	switch asset.MediaType {
	case common_models.ImageTypeJPEG, common_models.ImageTypePNG, common_models.ImageTypeGIF:
		return s.generateImageThumbnail(content, asset.MediaType)
	case common_models.VideoTypeMP4, common_models.VideoTypeMOV:
		if s.videoEnabled {
...
func (s *ThumbnailService) generateImageThumbnail(content []byte, mediaType common_models.MediaType) ([]byte, error) {
	// Decode image
	...
	switch mediaType {
	case common_models.ImageTypePNG:
...
func (s *ThumbnailService) generateVideoThumbnail(asset *common_models.PHAsset, content []byte) ([]byte, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
